...
function parseTableWithDom(document) {
  ...
  const reportDateIndex = findReportDateColumnIndex(headers.map((cell) => cell.textContent));
  ...
  return buildResult(totalHours, rowCount, invalidHourCount, invalidHourSamples, attendanceDates, dailyHoursMap);
}
...
function parseWithRegex(html) {
  ...
  const reportDateIndex = findReportDateColumnIndex(headerCells.map(stripHtml));
...
function buildResult(totalHours, rowCount, invalidHourCount, invalidHourSamples, attendanceDates, dailyHoursMap) {
  const result = {
...
function findReportDateColumnIndex(headers) {
  return headers.findIndex((header) => normalizeText(header) === REPORT_DATE_COLUMN_NAME);
}
